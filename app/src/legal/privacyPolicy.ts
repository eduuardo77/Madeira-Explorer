 ...
 *   4. **A recording the user chooses to send us** (D-069, Settings). The one
 *      way anything reaches us, and so the one exception to "never sent to us".
 *   5. **Where the position comes from.** On Android the fused provider is part
 *      of Google Play services, so the app cannot promise that Google "does not
 *      see your trip"; it can promise that *it* never sends it (review N6).
 *
 * ⚠ **Rewritten 2026-09-24 (T-216).** Besides N6's three sentences, four claims
 * had gone stale as the app changed underneath them: "two notifications, ever"
 * (the recording notice and T-210's update notice came later), "the one time
 * your trip leaves your phone" (the D-069 walk report), a map "left out of the
 * backup" (the MapLibre tile packs D-057 deleted), and a "short video" (T-105b is
 * not built). `privacyPolicy.test.ts` now fails on each of them.
 *
...
const SECTIONS_EN: PolicySection[] = [
  {
    ...
    paragraphs: [
      `${APP_NAME} records where you go while you are on holiday, and shows it back to you as a map. It keeps that record on your phone, not on a server.`,
