...
The complete reasoning for every decision, including rejected alternatives.
**`DECISIONS.md` is the index; this is the body.** Look a decision up here when
the index line is not enough — `grep -A40 "^## D-032" docs/decisions-full.md`.

Split out 2026-08-12: the index plus this file cost a session ~3k tokens to
orient in instead of ~45k to read. Nothing was cut. IDs are stable and are
never reused (CONTEXT §9).
